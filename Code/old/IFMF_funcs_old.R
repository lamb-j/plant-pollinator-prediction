...
# Factor gathering - combines list of factors into factor matrix
fact_gather <- function(x) {
  
  m <- length(x)
  if(m == 0) stop("zero user/item factors")  
  f <- length(x[[1]])
...
# Factor computation - computes the factor for the given user
  # x[[u]] = (Yt*Cuu*Y + lambda*I)^-1 * (Yt*Cuu*pu)
fact_comp <- function(Y, Cuu, lambda, pu) {
  Yt <- t(Y)          # transpose of Y
  I  <- diag(ncol(Y)) # Identity matrix
  
  YtCuu <- Yt%*%Cuu

  rv   <- solve(YtCuu%*%Y + lambda*I) %*%YtCuu%*%pu
  return(rv)
}
